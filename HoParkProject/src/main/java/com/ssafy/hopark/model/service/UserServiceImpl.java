...
public class UserServiceImpl implements UserService {
	...
	// BCryptPasswordEncoder 는 해싱 시 원본과 상관 없이 60자 이하의 문자열이 반환됨. salt도 자동으로 추가해줌 -> 이
	// 값을 DB에 저장
	// BCryptPasswordEncoder 는 해싱할 때마다 다른 값이 나와서 비밀번호 검증을 위한 메서드를 따로 제공하고 있음
	// (matches)
	@Autowired
	...
	@Override
	public User getInfo(String principal) {
		return userDao.selectOne(principal);
	}

	@Override
	public boolean updateWork(Work work) {
		int res = userDao.updateWork(work);
		if(res==0) {
			return false;
		}
		return true;
