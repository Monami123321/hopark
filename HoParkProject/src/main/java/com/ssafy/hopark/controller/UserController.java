...
//		System.out.println(header.toString() + "헤더임");
	...
	@ApiOperation(value = "로그인 된 회원의 정보를 반환합니다.", notes = "로그인 상태인 유저의 정보를 DB에서 조회하여 반환합니다. SecurityContextHolder에서 Authentication에 저장된 principal(id) 기준으로 조회합니다.")
	@GetMapping("info")
	public ResponseEntity<?> getLoggedinUserInfo() {
		...
		User user = userService.getInfo(principal);
		if (user == null) {
			return new ResponseEntity<String>(FAIL, HttpStatus.NOT_ACCEPTABLE);
		}
	...
	@ApiOperation(value = "설문조사에서 선택한 4개 운동 정보를 입력합니다.", notes = "로그인 상태인 유저의 정보를 조회하여 DB에 운동목록을 반영합니다.")
	@PutMapping("work")
	public ResponseEntity<?> updateWork(@RequestBody Work work) {
		...
		work.setName(principal);
		boolean res = userService.updateWork(work);
